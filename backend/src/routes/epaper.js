...
const PAPER_RSS = {
  ...
  visalaandhra:       'https://www.visalaandhra.com/feed',
...
// ── Direct epaper website URLs (for "Visit E-Paper" button) ──────────────────
...
// ── GET /api/epaper/:paperId/articles?date=YYYY-MM-DD&keyword= ───────────────
  ...
  try {
    ...
    if (keyword && keyword.trim()) {
      ...
      const matched = items.filter(a =>
        a.title.toLowerCase().includes(kw) || a.snippet.toLowerCase().includes(kw)
      );
...
// ── GET /api/epaper/meta — return all paper epaper URLs ──────────────────────
router.get('/meta', (req, res) => {
  res.json({ PAPER_EPAPER_URL });
});
